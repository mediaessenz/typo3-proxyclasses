...
class ClassCacheManager
{
    ...
    public function __construct($extensionKey)
    {
        ...
        $this->cacheInstance = $classLoader->initializeCache($extensionKey);
    }
     ...
     * @throws \Exception
     */
    public function reBuild($extensionKey)
    {
        $classPath = 'Classes/';

        foreach ($GLOBALS['TYPO3_CONF_VARS']['EXT'][$extensionKey]['classes'] as $key => $extensionsWithThisClass) {
            ...
            $code = self::parseSingleFile($path, true);

            // Get the files from all other extensions
            foreach ($extensionsWithThisClass as $extensionWithThisClass) {
                $path = ExtensionManagementUtility::extPath($extensionWithThisClass) . $classPath . $key . '.php';
                if (is_file($path)) {
                    $extendingClassFound = true;
                    $code .= self::parseSingleFile($path, false);
                }
            }
            $code = self::closeClassDefinition($code);

            // If an extending class is found, the file is written and
            // added to the autoloader info
            if ($extendingClassFound) {
                $cacheEntryIdentifier = 'tx_' . $extensionKey . '_' . strtolower(str_replace('/', '_', $key));
                try {
     ...
     */
    protected static function parseSingleFile($filePath, $baseClass = false)
    {
        if (!is_file($filePath)) {
        ...
        if ($baseClass) {
            ...
            $content = self::getPartialInfo($filePath) . substr($codePart, 0, $closingBracket);
            return $content;
        }
    }
     ...
     * @return string
     */
    protected static function getPartialInfo($filePath)
    {
     ...
     * @return string
     */
    protected static function closeClassDefinition($code)
    {
        return $code . LF . '}';
    }
}
